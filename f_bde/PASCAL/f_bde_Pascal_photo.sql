CREATE TABLE f_bde_Pascal_photo AS
(SELECT *
 FROM
    XMLTable('/photos/photo'
      passing xmltype(
      BFILENAME('REPO_DATA','photosPASCAL.xml'),
...
Select * from f_bde_Pascal_photo where rownum=1;
Select count(*) from f_bde_Pascal_photo;
