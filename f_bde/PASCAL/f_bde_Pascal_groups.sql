CREATE TABLE f_bde_Pascal_groups_pool AS
(SELECT p.photo_id, g.group_id, g.group_title, g.group_url
FROM f_bde_Pascal_photo p,
...
Select * from f_bde_Pascal_groups_pool where rownum=1;
Select count(*) from f_bde_Pascal_groups_pool;

CREATE TABLE f_bde_Pascal_groups_set PARALLEL COMPRESS AS
(SELECT p.photo_id,
...
Select * from f_bde_Pascal_groups_set where rownum=1;
Select count(*) from f_bde_Pascal_groups_set;
